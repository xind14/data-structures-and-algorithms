...
const count = (target, input) => {
  ...
  return input.reduce((count, array)=> {
    let occurrences = array.reduce((array,number)=> {
return array + (number === target?1:0);
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  return input.map((array)=>{
    return array.filter((number)=>typeof number==='number' && number % 5===0).map((number)=>Math.pow(2, number));
  })
};
...
------------------------------------------------------------------------------------------------ */
//use gpt couldn't remember parse
